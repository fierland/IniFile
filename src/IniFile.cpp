...
{
	ESP_LOGD("IniFile", "start cpy filename ");
	if (strlen(filename) <= maxFilenameLen)
		strcpy(_filename, filename);
	else
		_filename[0] = '\0';

	ESP_LOGD("IniFile", "start cpy mode new =%s", mode);

	if (mode != NULL)
		strcpy(_mode, mode);

	ESP_LOGD("IniFile", "constructor done", mode);
	...
	char* buffer, size_t len, IniFileState &state) const
{
	...
	ESP_LOGD(TAG, "Start search for [%s] <%s>", (section != NULL ? section : "NULL"), key);

	switch (state.getValueState)
	{
	case IniFileState::funcUnset:
		ESP_LOGD(TAG, "funcUnset");
	...
	char* buffer, size_t len) const
{
	...
	while (!getValue(section, key, buffer, len, state))
		;
	ESP_LOGD(TAG, "found :%s: erro %d", buffer, _error);
...
IniFile::error_t IniFile::readLine(FILE* file, char *buffer, size_t len, uint32_t &pos)
{
	...
	//ESP_LOGD(TAG, "file seek to position %d", pos);
	...
	if (fgets(buffer, len, file) == NULL)
	{
		buffer[0] = '\0';
		//return 1; // done
		ESP_LOGD(TAG, "eof reached");
		return errorEndOfFile;
	}

	int bytesRead = strlen(buffer);
	ESP_LOGV(TAG, "Line read %d :%s:", bytesRead, buffer);
...
{
	//ESP_LOGV(TAG, "str = :%s: len=%d", str, strlen(str));
	char *cp = str + strlen(str) - 1;
	while (cp >= str && ((*cp == ' ') || (*cp == '\t')))
	{
		//ESP_LOGV(TAG, "pos = %d val=:%c:", cp, *cp);
		*cp-- = '\0';
	}
}
	...
	IniFileState &state) const
{
	...
	{
		ESP_LOGD(TAG, "Line read failed");
		// Signal to caller to stop looking and any error value
		_error = err;
		return true;
	}
	//ESP_LOGD(TAG, "Line read done :%s", buffer);
		...
		if (ep != NULL)
		{
			*ep = '\0'; // make ] be end of string
			...
			if (_caseSensitive)
			{
				...
				{
					ESP_LOGD(TAG, "section found CaseSensitive [%s]", section);
					_error = errorNoError;
					return true;
				}
			}
			else
			{
				...
				{
					ESP_LOGD(TAG, "section found [%s]", section);
					_error = errorNoError;
					return true;
				}
			}
		...
		*ep = '\0'; // make = be the end of string
		...
		if (_caseSensitive)
		{
			if (strcmp(cp, key) == 0)
			{
				*keyptr = ep + 1;
				_error = errorNoError;
				ESP_LOGD(TAG, "key found Case Sensitive <%s>", key);
				return true;
			}
		}
		else
...
bool IniFile::open(void)
{
	...
	ESP_LOGD("IniFile", "Open file %s in mode %s", _filename, _mode);
